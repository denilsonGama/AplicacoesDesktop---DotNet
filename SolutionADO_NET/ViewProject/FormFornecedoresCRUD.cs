    ...
    public partial class FormFornecedoresCRUD : Form {
        ...
        private Fornecedor GetFornecedorById(long id)
        {
            ...
            using (SqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    fornecedor.Id = Convert.ToInt32(reader[0]); //Alterado. Original: reader.GetInt32(0)
        ...
        private void dgvFornecedores_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0 || e.ColumnIndex < 0)
                return;
            this.fornecedorAtual = GetFornecedorById(Convert.ToInt32(dgvFornecedores.Rows[e.RowIndex].Cells[0].Value));
